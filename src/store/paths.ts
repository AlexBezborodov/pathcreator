...
class pathsBuilder {
  ...
  removePath(pathId: number) {
    const filteredArray = this.pathsStore.filter(
      (item: DataPath) => item.id !== pathId,
    );
    this.pathsStore = filteredArray;
  }

  addToFavourites(pathId: number) {
    const updatedArray = this.pathsStore.map((item: any) =>
      item.id === pathId ? { ...item, isFavourite: !item.isFavourite } : item,
    );
